...
module.exports = function(sender, receiver, connector) {
  return Promise.all([sender.connect(), receiver.connect()]).then(() => {
    ...
    sender.on('outgoing_fulfill', (transfer, fulfillment) => { console.log('test success!', transfer, fulfillment); process.exit(0) })
    sender.on('outgoing_reject', (transfer, reason) => { console.log('test failed by connector!', transfer, reason); process.exit(1) })
    sender.on('outgoing_cancel', (transfer, reason) => { console.log('test failed by ledger!', transfer, reason); process.exit(1) })
